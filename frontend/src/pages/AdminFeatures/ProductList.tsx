...
export default function ProductList() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.error(err);
      // Show a friendlier message for 401s
  ...
  useEffect(() => {
    // ⛔️ Don’t run until auth state is rehydrated
    if (!rehydrated) return;

    // If not logged in, you can redirect or show an error
  ...
  const handleDelete = async (productId: string) => {
    ...
    try {
      ...
      await axios.delete(`/api/admin/products/${productId}/`, { headers: authHeader });
  ...
  // While we’re waiting for rehydration, keep the spinner up
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm mb-2">
                  ...
                  {product.is_featured && (
                    <span className="bg-pink-400/20 text-pink-300 px-2 py-1 rounded-full">Featured</span>
                  )}
                </div>
                ...
                <div className="text-sm mb-2">
                  ...
                  {product.available_colors_list && product.available_colors_list.length > 3 ? "..." : ""}
                </div>

                <div className="flex justify-between items-center text-sm mb-4">
                  <span className="text-white/80">स्टक: {product.stock_quantity}m</span>
